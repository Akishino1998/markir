...
<div class="main-content-container container-fluid px-4">
  <div class="page-header row no-gutters py-4">
    ...
    <div class="col-12 col-sm-4 d-flex align-items-center">
      <div class="btn-group btn-group-sm btn-group-toggle d-inline-flex mb-4 mb-sm-0 mx-auto" role="group" aria-label="Page actions">
        <a href="/riwayat" class="btn btn-white"> All  </a>
        @foreach ($jenis as $item)
          @if ($id == $item->id_ref_kendaraan)
            <a href="/riwayat/jenis/{{ $item->id_ref_kendaraan }}" class="btn btn-white active"> {{ $item->jenis_kendaraan }}  </a>
          @else
    ...
    <div class="col-12 col-sm-4"  style="float:right;">
      <form action="/riwayat/date/{{ $id }}" method="post">
        @csrf
        ...
        <input type="text" value="{{ $id }}" name="id_jenis" hidden>
      </form>
    </div>
  ...
  <div class="row">
    ...
    @if (isset($request))
      <div class="col-md-12">
        <h6>{{ $request->start }} - {{ $request->end }}</h6> 
      </div>
    @endif
    <?php $i=0; ?>
